(function ($, Backbone, _, app) {
    ...
    var FormView = TemplateView.extend({
        ...
        submit: function (event) {
            event.preventDefault();  // 阻止触发form的默认事件
            this.form = $(event.currentTarget);  // 指向当前绑定事件的元素，即<from></form>
            this.clearErrors();
        },
    ...
    var TaskItemView = TemplateView.extend({
        ...
        drop: function (event) {
            ...
            if (task !== this.task) {
                ...
                let order = this.task.get('order');
                let tasks = app.tasks.filter(function (model) {
    ...
    var SprintView = TemplateView.extend({  // sprint detail view
        ...
        initialize: function (options) {
            ...
            // Monitor the `drop` event in every instance of StatusView,
            // when the event fires, then make use of ws to broadcast messages.
